...
export const initDataSource = (dataSources: any) => {
  ...
  const globalErrorHandler = dataSources.errorHandler ? createFn(dataSources.errorHandler.value) : (err: any) => Promise.reject(err)
  ...
  const load = (http: any, options: any, dataSource: any, shouldFetch: any) => (params: any, path: any, customConfig: any) => {
